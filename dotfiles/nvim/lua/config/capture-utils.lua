...
function M.capture_buffer_as_image()
    ...
    vim.fn.jobstart(cmd, {
        on_exit = function(_, code)
            if code == 0 then
                safe_notify("Screenshot saved to: " .. output_file, vim.log.levels.INFO, {
                    title = "Code Capture",
                    timeout = 3000,
                })
            else
                safe_notify("Failed to capture screenshot", vim.log.levels.ERROR, {
                    title = "Code Capture",
                    timeout = 3000,
                })
            end
...
function M.capture_selection_as_image()
    ...
    vim.fn.jobstart(cmd, {
        on_exit = function(_, code)
            if code == 0 then
                safe_notify("Selection captured to: " .. output_file, vim.log.levels.INFO, {
                    title = "Code Capture",
                    timeout = 3000,
                })
            else
                safe_notify("Failed to capture selection", vim.log.levels.ERROR, {
                    title = "Code Capture",
                    timeout = 3000,
                })
            end
...
function M.start_screen_recording()
    ...
    M.recording_pid = vim.fn.jobstart(cmd, {
        on_exit = function(_, code)
            if code == 0 then
                safe_notify("Recording saved to: " .. output_file, vim.log.levels.INFO, {
                    title = "Screen Recording",
                    timeout = 3000,
                })
            else
                safe_notify("Recording stopped", vim.log.levels.INFO, {
                    title = "Screen Recording",
                    timeout = 3000,
                })
            end
    ...
    safe_notify("Screen recording started. Press <leader>cr to stop.", vim.log.levels.INFO, {
        title = "Screen Recording",
        timeout = 3000,
    })
end
...
function M.stop_screen_recording()
    if M.recording_pid then
        ...
        M.recording_pid = nil
        safe_notify("Screen recording stopped", vim.log.levels.INFO, {
            title = "Screen Recording",
            timeout = 2000,
        })
    else
        safe_notify("No active recording found", vim.log.levels.WARN, {
            title = "Screen Recording",
            timeout = 2000,
        })
    end
...
function M.capture_code_block()
    ...
    if f then
        ...
        vim.fn.jobstart(cmd, {
            on_exit = function(_, code)
                if code == 0 then
                    safe_notify("Code block captured to: " .. output_file, vim.log.levels.INFO, {
                        title = "Code Capture",
                        timeout = 3000,
                    })
                else
                    safe_notify("Failed to capture code block", vim.log.levels.ERROR, {
                        title = "Code Capture",
                        timeout = 3000,
                    })
                end
